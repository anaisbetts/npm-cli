...
function npa (arg, where) {
  ...
  // Handle workspace: URLs before standard URL processing
  if (/^workspace:/i.test(arg)) {
    spec = arg
  } else if (isURL.test(arg)) {
    spec = arg
...
function resolve (name, spec, where, arg) {
  ...
  // Handle workspace: protocol by converting to file: protocol
  if (spec && /^workspace:/i.test(spec)) {
    log.warn('npm-package-arg', `Encountered workspace: protocol for ${name}, converting to file: protocol`)
    const packageName = name.split('/').pop()
    const convertedSpec = `file:../${packageName}`
    res.rawSpec = convertedSpec
    res.raw = name ? `${name}@${convertedSpec}` : convertedSpec
    spec = convertedSpec
  }
